...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const cap = (segment: string) =>
        segment.charAt(0).toUpperCase() + segment.slice(1);
      const dimensionKey = cap(key);

      if (width <= 480 && config[`mobile${dimensionKey}` as keyof GradualBlurProps]) {
        next = config[`mobile${dimensionKey}` as keyof GradualBlurProps] as
          | string
          | undefined;
      } else if (
        width <= 768 &&
        config[`tablet${dimensionKey}` as keyof GradualBlurProps]
      ) {
        next = config[`tablet${dimensionKey}` as keyof GradualBlurProps] as
          | string
          | undefined;
      } else if (
        width <= 1024 &&
        config[`desktop${dimensionKey}` as keyof GradualBlurProps]
      ) {
        next = config[`desktop${dimensionKey}` as keyof GradualBlurProps] as
          | string
          | undefined;
      }
...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (props) => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      baseStyle.height = responsiveHeight as string | undefined;
      baseStyle.width = (responsiveWidth as string | undefined) || "100%";
      baseStyle[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width =
        (responsiveWidth as string | undefined) ||
        (responsiveHeight as string | undefined);
