    ...
    <SheetPortal {...portalProps}>
      ...
      <SheetViewport side={side} variant={variant}>
        <SheetPrimitive.Popup
          className={cn(
            "relative flex max-h-full min-h-0 w-full min-w-0 flex-col bg-popover not-dark:bg-clip-padding text-popover-foreground shadow-lg/5 outline-none transition-[opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:shadow-[0_1px_--theme(--color-black/4%)] focus:outline-none focus-visible:outline-none focus-visible:ring-0 data-ending-style:opacity-0 data-starting-style:opacity-0 max-sm:before:hidden dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            side === "bottom" &&
...
export function SheetPanel({
  ...
  return (
    <ScrollArea
      ...
      className="**:data-[slot=scroll-area-viewport]:focus-visible:ring-0 **:data-[slot=scroll-area-viewport]:focus-visible:ring-offset-0"
    >
      {useRender({
        defaultTagName: "div",
        props: mergeProps<"div">(defaultProps, props),
